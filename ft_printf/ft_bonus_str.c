...
static void	flag_specific(t_p *p, t_fl *f, char *str)
{
	...
	if (f->dash == true)
	{
		p->i += ft_cpy_num(p->str + p->i, str, f->width);
	}
	if (spaces > 0)
	{
		p->i += ft_cpy_n_chr(p->str + p->i, ' ', spaces);
	}
	if (f->dash == false)
	{
		p->i += ft_cpy_num(p->str + p->i, str, f->width);
...
static void	str_bonus_trun(t_p *p, t_fl *f)
{
	...
	if (null_check == NULL)
	{
		...
		{
			p->i += ft_cpy_n_chr(p->str + p->i, ' ', f->width - f->prec);
			p->i += ft_cpy_str(p->str + p->i, "(null)");
		}
		else
		{
...
void	str_bonus(t_p *p, t_fl *f)
{
	...
	else if (f->dot == false && f->ap_len < f->width)
	{
		if (f->dash == true)
			spec_str_one(p, f);
		p->i += ft_cpy_n_chr(p->str + p->i, ' ', f->width - f->ap_len);
		if (f->dash == false)
			spec_str_one(p, f);
	}
	else if (f->dot == true)
