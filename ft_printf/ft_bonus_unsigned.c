...
static void	before_or_after(t_p *p, t_fl *f, bool stop, short nil)
{
	...
	{
		p->i += ft_cpy_str(p->str + p->i, f->prefix);
		stop = true;
	}
	if (f->width >= 0 && f->dash == false && f->zero == true && f->dot == false)
		p->i += ft_cpy_n_chr(p->str + p->i, '0', f->width - f->hsh);
	if (f->dot == true)
		p->i += ft_cpy_n_chr(p->str + p->i, '0', f->prec - f->ap_len);
	...
	if (f->width >= 0 && f->dash == true)
		p->i += ft_cpy_n_chr(p->str + p->i, ' ', f->width - f->hsh - nil);
}
...
void	nbr_bonus(t_p *p, t_fl *f)
{
	...
	if (f->width >= 0 && f->dash == false
		...
		p->i += ft_cpy_n_chr(p->str + p->i, ' ', f->width - f->hsh - nil_check);
